...
async fn register(State(st): State<AppState>, Json(b): Json<RegisterBody>) -> Result<Json<Value>, AppError> {
    ...
    .await?;
    // Authorize: only the project owner may register (extends to admins under D5).
    if research::project_owner(&st.pool, b.project_id).await?.as_deref() != Some(b.steward_did.as_str()) {
...
async fn subjects(State(st): State<AppState>, Query(q): Query<SubjectsQuery>) -> Result<Json<Value>, AppError> {
    ...
    if !research::is_project_participant(&st.pool, &q.did, q.project_id).await? {
        return Err(AppError::Forbidden);
    }
